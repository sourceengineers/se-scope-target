...
TEST(msgpack_unpacker, test_fetcher_functions)
{
  ...
  CommandFetchingInformation information = {.commandName = (char*) "cf_tgr",
                                            .fieldName = (char*) "cl_id",
                                            .isInArray = false,
                                            .arrayIndex = 0};
  
  int channelId = parser->getIntFromCommand(parser, &information);
  EXPECT_EQ(channelId, 1);

  char fetchedData[20];
  information.fieldName = (char*)  "mode";
  parser->getStringFromCommand(parser, &information, fetchedData, 20);
  EXPECT_STREQ(fetchedData, "Continous");
  
  information.fieldName = (char*) "level";
  float level = parser->getFloatFromCommand(parser,  &information);
  EXPECT_EQ(level, 2.5f);

  information.fieldName = (char*) "edge";
  bool edge = parser->getBoolFromCommand(parser, &information);
  EXPECT_EQ(edge, true);

  information.commandName = (char*) "cf_t_inc";
  information.fieldName = (char*) "";
  int stamp = parser->getIntFromCommand(parser, &information);
  EXPECT_EQ(stamp, 10);
}
...
TEST(msgpack_unpacker, test_automatic_mode)
{
  ...
  CommandFetchingInformation information = {.commandName = commandName,
                                            .fieldName = fieldName,
                                            .isInArray = false,
                                            .arrayIndex = 0};

  bool setRunning = parser->getBoolFromCommand(parser, &information);
  EXPECT_EQ(setRunning, false);

  information.fieldName = (char*) "2";
  setRunning = parser->getBoolFromCommand(parser, &information);
...
TEST(msgpack_unpacker, test_errors)
{
  ...
  information.fieldName = (char*) "lavel";
  level = parser->getFloatFromCommand(parser, &information);
  EXPECT_EQ(level, 0.0f);

  information.commandName = (char*) "cf_tgri";
  level = parser->getFloatFromCommand(parser, &information);
  EXPECT_EQ(level, 0.0f);
